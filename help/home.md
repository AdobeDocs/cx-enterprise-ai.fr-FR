...
Pour voir le Chat des collègues en action, consultez [Chat des collègues dans Playground](./coworker/playground-coworker-chat.md) ou lisez des cas d’utilisation réels tels que [Valider des données de migration d’AA vers CJA](./coworker/data-validation-aa-cja.md) et [Analyser le taux de déperdition de la conversion](./coworker/analytics-use-cases.md).
