---
title: Créer des défis et des informations en matière de fidélité
description: Découvrez comment Coworker contribue à créer et à gérer des défis de fidélité, à automatiser la sélection d’audiences et à obtenir des informations sur la surface.
role: User
level: Beginner
doc-type: Feature Video
duration: 499
last-substantial-update: 2026-08-12T00:00:00.000Z
jira: KT-22312
product_v2:
  - id: fdae8433-07cd-42e7-acce-738afe63f6bb
    internal-label: CX Enterprise Coworker
feature_v2:
  - id: fdae8433-07cd-42e7-acce-738afe63f6bb
    internal-label: CX Enterprise Coworker
source-git-commit: 4bd1bca0d5f967eaf33802b8d955aa89767b662a
workflow-type: tm+mt
source-wordcount: '140'
ht-degree: 1%
---

# Adobe Journey Optimizer &amp; Coworker : créez un défi de fidélité et obtenez des informations

Dans cette démonstration, vous découvrirez comment la fidélité Adobe Journey Optimizer (AJO) est utilisée dans Coworker pour simplifier et accélérer la gestion du programme de fidélité.
 
La vidéo présente le plug-in de fidélité et ses compétences de gestion des défis, une fonctionnalité optimisée par l’IA qui aide les spécialistes marketing à créer, modifier et gérer les défis de fidélité à l’aide du langage naturel au lieu de configurer manuellement chaque paramètre de l’interface utilisateur.

>[!VIDEO](https://video.tv.adobe.com/v/3496528/?learn=on&enablevpops)

## Voir également

- [&#128279;](../../../../agents/ajo-agent.md), pour d’autres fonctionnalités de l’IA d’AJO disponibles via le Module de conversation des collègues, telles que la création, l’analyse et la simulation de parcours.
- [Loyalty Challenge Management](https://experienceleague.adobe.com/fr/docs/journey-optimizer/using/get-started/essentials/ajo-coworker-skills#loyalty-challenge-management){target="_blank"} dans la documentation de Adobe Journey Optimizer.
- [Plus de cas d’utilisation de la conversation avec un collègue](../overview.md#journeys)
